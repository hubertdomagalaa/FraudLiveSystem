...
from shared.schemas.agents import ContextAgentOutput, PolicyAgentRequest
from shared.schemas.transactions import TransactionStored
from shared.worker import StreamWorker


class PolicyStreamWorker:
    def __init__(self, settings: PolicySettings, db: PlatformDatabase, broker: RedisStreamBroker) -> None:
        ...
        self._worker = StreamWorker(
            ...
            stream_name=StreamName.AGENT_POLICY_COMMANDS,
    ...
    async def start(self) -> None:
        self._task = asyncio.create_task(self._worker.run(self._stop_event), name="policy-stream-worker")
    ...
    async def _handle(self, event, message_id: str) -> None:
        ...
        if event.event_type != EventType.STEP_RUN_REQUESTED:
            return
        if event.payload.get("step") != StepName.POLICY:
            return
        ...
        risk_payload = input_payload.get("risk", {})
        ...
        completed_event = build_event(
            ...
            event_id=deterministic_uuid(event.case_id, EventType.AGENT_POLICY_COMPLETED, event.event_id),
            idempotency_key=f"{event.case_id}:policy:{event.event_id}",
        ...
        await self.db.append_agent_run(
            agent_run_id=deterministic_uuid(event.case_id, "agent-run-policy", event.event_id),
            case_id=event.case_id,
            agent_name=self.settings.service_name,
            step_name=StepName.POLICY,
            attempt=event.attempt,
            status="COMPLETED",
            started_at=response.metadata.started_at,
            finished_at=response.metadata.completed_at,
            latency_ms=response.metadata.latency_ms,
            agent_version=self.settings.ruleset_version,
