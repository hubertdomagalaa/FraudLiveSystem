...
from app.stream_worker import AggregateStreamWorker
...
async def lifespan(app: FastAPI):
    ...
    worker = AggregateStreamWorker(settings=settings, db=db, broker=broker)
    await worker.start()
...
app = FastAPI(title="Aggregate Agent", version="1.0.0", lifespan=lifespan)
