...
from app.stream_worker import ExplainStreamWorker
...
async def lifespan(app: FastAPI):
    ...
    worker = ExplainStreamWorker(settings=settings, db=db, broker=broker)
    await worker.start()
...
app = FastAPI(title="LLM Explanation Agent", version="1.0.0", lifespan=lifespan)
