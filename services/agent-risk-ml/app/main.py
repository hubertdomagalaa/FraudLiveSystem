...
from app.stream_worker import RiskStreamWorker
...
async def lifespan(app: FastAPI):
    ...
    worker = RiskStreamWorker(settings=settings, db=db, broker=broker)
    await worker.start()
...
app = FastAPI(title="Risk ML Agent", version="1.0.0", lifespan=lifespan)
